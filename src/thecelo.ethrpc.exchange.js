...
async function get_exchange_history() {
  ...
  if (records && records.length > 0) {
    ...
    if (exchange_records.length > 0) {
      // next block begin
      fromHeight =
        "0x" + (parseInt(exchange_records[0].blockNumber) + 1).toString(16);
  ...
  result.forEach((item, i) => {
    ...
    //
    if (
      parseFloat(sellAmount / 1e18) > 0.0001 &&
      parseFloat(buyAmount / 1e18) > 0.0001
    ) {
      if (blockNumber != item.blockNumber) {
        blockNumber = item.blockNumber;
        let block = thecelo.eth_rpc(
  ...
  redis.redis_client.set(
    ...
    JSON.stringify(exchange_records)
  );
  //
...
async function update_exchange_prices() {
  ...
  try {
    ...
    redis.redis_client.set(
      "exchange_prices",
      JSON.stringify(exchange_prices_list)
    );
    //get bittrex
    var bittrex_url = "https://api.bittrex.com/v3/markets/CELO-USDT/ticker";
    req.get({ url: bittrex_url, encoding: "utf8" }, function (
      err,
      res,
      content
    ) {
      if (!err) {
        ...
        redis.redis_client.set(
          "bittrex_prices",
          JSON.stringify(bittrex_prices_list)
        );
      }
    });
    //get bittrex
    ...
    req.get({ url: bittrex_day_url, encoding: "utf8" }, function (
      err,
      res,
      content
    ) {
      if (!err) {
        ...
        day_prices.forEach((item, i) => {
          ...
          if (week != pre_week) {
            ...
            pre_prices.startsAt =
              pre_prices.startsAt.substr(0, 5) +
              "~" +
              item.startsAt.substr(5, 5);
          }
        });
        //
...
async function get_k_line(k_time) {
    ...
    if (Object.keys(prices_data_rows).length < 50) {
      ...
      if (k_time <= 60) {
        time_span =
          ...
          YMDhms[3] +
          ":" +
          parseInt(YMDhms[4] / k_time) * k_time;
      } else {
      ...
      volume = parseFloat((volume / 1e18).toFixed(2));
  ...
  redis.redis_client.set(
    ...
    JSON.stringify(prices_data_rows)
  );
