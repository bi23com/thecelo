...
//
var validatorGroupVoteCast_input = [
  { type: "address", name: "account", indexed: true },
  { type: "address", name: "group", indexed: true },
  { type: "uint256", name: "value", indexed: false },
];
var validatorGroupVoteCast = web3.eth.abi.encodeEventSignature({
  type: "event",
  name: "ValidatorGroupVoteCast",
  inputs: validatorGroupVoteCast_input,
});
//
var validatorGroupVoteActivated_input = [
  { type: "address", name: "account", indexed: true },
  { type: "address", name: "group", indexed: true },
  { type: "uint256", name: "value", indexed: false },
  { type: "uint256", name: "units", indexed: false },
];
var validatorGroupVoteActivated = web3.eth.abi.encodeEventSignature({
  type: "event",
  name: "ValidatorGroupVoteActivated",
  inputs: validatorGroupVoteActivated_input,
});
//
var validatorGroupActiveVoteRevoked_input = [
  { type: "address", name: "account", indexed: true },
  { type: "address", name: "group", indexed: true },
  { type: "uint256", name: "value", indexed: false },
  { type: "uint256", name: "units", indexed: false },
];
var validatorGroupActiveVoteRevoked = web3.eth.abi.encodeEventSignature({
  type: "event",
  name: "ValidatorGroupActiveVoteRevoked",
  inputs: validatorGroupActiveVoteRevoked_input,
});
//
var validatorGroupPendingVoteRevoked_input = [
  { type: "address", name: "account", indexed: true },
  { type: "address", name: "group", indexed: true },
  { type: "uint256", name: "value", indexed: false },
];
var validatorGroupPendingVoteRevoked = web3.eth.abi.encodeEventSignature({
  type: "event",
...
function elected_minimum_votes(cmd, blockNumber) {
  ...
  lines.forEach(function (line) {
    if (line.indexOf("0x") == 0) {
      ...
      if (groups[affiliation]) groups[affiliation][0]++;
      else groups[affiliation] = [1, 0, 0];
    }
  });
  //
  Object.keys(groups).forEach((address, i) => {
    let activeVotesForGroup = eth_call_0(
      electionproxy_address,
...
function latest_epoch_election_votes() {
  ...
  let current_elected_minimum_votes = elected_minimum_votes(
    "celocli election:current",
    prev_epoch_blockNumber
  );
  let frontrunner_elected_minimum_votes = elected_minimum_votes(
    "celocli election:run",
    "latest"
  );
  //
  let result = {
    current_elected_minimum_votes,
    frontrunner_elected_minimum_votes,
    votes,
  };
...
function election_vote(
  ...
  //
  votes.sort(function compareNumbers(a, b) {
    return a[3] - b[3];
  });
...
function op_weight(votes, voteds, voters, result, flg) {
  result.forEach((item, i) => {
    var from =
      "0x" + item["topics"][1].replace("0x000000000000000000000000", "");
    ...
    if (flg == 0) {
      ...
      var ret = web3.eth.abi.decodeLog(
        validatorGroupVoteActivated_input,
        item["data"],
        item["topics"]
      );
      ...
      var ret = web3.eth.abi.decodeLog(
        validatorGroupActiveVoteRevoked_input,
        item["data"],
        item["topics"]
      );
    ...
    //
    var weight =
      cast_weight + activated_revoked_weight + pending_revoked_weight;
    votes.push([
      from,
      to,
      weight,
      block,
      activated_weight,
      activated_revoked_weight,
      pending_revoked_weight,
      cast_weight,
    ]);
    //
    if (voteds[to]) {
      if (voteds[to][from]) voteds[to][from].weight += weight;
      else voteds[to][from] = { weight };
    } else {
      var voted = {};
      voted[from] = { weight };
      voteds[to] = voted;
    ...
    //
    if (voters[from]) {
      if (voters[from][to]) voters[from][to].weight += weight;
      else voters[from][to] = { weight };
    } else {
      var voter = {};
      voter[to] = { weight };
      voters[from] = voter;
...
function eth_call_0(contract, method, address, datatype, blockNumber) {
  ...
  var result = thecelo.eth_rpc(
    "eth_call",
    '[{"to": "' + contract + '", "data":"' + data + '"}, "' + blockNumber + '"]'
  );
  //console.log(result);
...
function getGroupVotesStatus(group, blockNumber = "latest") {
  ...
  var totalVotesForGroup = eth_call_0(
    electionproxy_address,
    "getTotalVotesForGroup",
    group,
    "uint256",
  ...
  var activeVoteUnitsForGroup = eth_call_0(
    electionproxy_address,
  ...
  var activeVotesForGroup = eth_call_0(
    electionproxy_address,
  ...
  var pendingVotesForGroup = eth_call_0(
    electionproxy_address,
  ...
  var groupEligibility = eth_call_0(
    electionproxy_address,
    "getGroupEligibility",
    group,
    "uint256",
  ...
  var canReceiveVotes = eth_call_1(
    electionproxy_address,
    "canReceiveVotes",
    group,
    1,
  ...
  var numVotesReceivable = eth_call_0(
    electionproxy_address,
  ...
  return {
    totalVotesForGroup,
    activeVoteUnitsForGroup,
    activeVotesForGroup,
    pendingVotesForGroup,
    groupEligibility,
    canReceiveVotes,
    numVotesReceivable,
  };
...
function getEpochNumber() {
  ...
  var result = thecelo.eth_rpc(
    "eth_call",
    '[{"to": "' + electionproxy_address + '", "data":"' + data + '"}, "latest"]'
  );
...
function epochRewardsDistributedToVoters(group) {
  ...
  var event_topic = web3.eth.abi.encodeEventSignature({
    type: "event",
  ...
  result.forEach((item, i) => {
    epoch_rewards[
      Math.ceil(item.blockNumber / theceloconst.EPOCH_SIZE)
    ] = parseInt(item.data);
  });
