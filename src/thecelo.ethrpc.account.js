  ...
  try {
    ...
    if (metadata && metadata.claims) {
      for (var i = 0; i < metadata.claims.length; i++) {
        if (metadata.claims[i].hasOwnProperty("domain"))
          domain = metadata.claims[i].domain;
        if (metadata.claims[i].type == "ATTESTATION_SERVICE_URL")
          attestation_service_url = metadata.claims[i].url;
        if (metadata.claims[i].type == "KEYBASE")
          keybase = metadata.claims[i].username;
      }
    }
  } catch (err) {
    console.log(err);
  ...
  Object.keys(groups_metadata).forEach((address, i) => {
    if (groups_metadata[address].keybase == name) {
      ...
      if (domain.trim().length > 0) {
        var aa = domain.split(".");
        if (aa[aa.length - 2] == name) result = address;
      }
    }
  });
  console.log(result);
