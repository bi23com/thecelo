...
function validatorgroup_register() {
  var result = thecelo.eth_rpc(
    ...
    '[{"fromBlock": "earliest","toBlock":"latest","address":"' +
      validatorsproxy_address +
      '","topics":["' +
      validatorgroup_register_topic +
      '"]}]'
  );
}
//
...
function validatorEpochPaymentDistributed(validator, group) {
  ...
  var validatorEpochPaymentDistributed = web3.eth.abi.encodeEventSignature({
    type: "event",
  ...
  result.forEach((item, i) => {
    ...
    epoch_payments[key] = [
      parseInt(data.validatorPayment) / 1e18,
      parseInt(data.groupPayment) / 1e18,
    ];
  });
...
function validatorScoreUpdated(validator) {
  ...
  var validatorScoreUpdated = web3.eth.abi.encodeEventSignature({
    type: "event",
...
var subscription = web3ws.eth
  .subscribe("newBlockHeaders", function (error, result) {
    if (!error) {
      ...
      //
      if (
        Object.keys(signers_groups).length == 0 ||
      ...
      redis.redis_client.publish(
        theceloconst.eth_blockdata_key,
        JSON.stringify(eth_blockdata),
        function (err, result) {}
      );
      return;
    }
...
module.exports = {
  ...
  getMembershipInLastEpochFromSigner,
};
