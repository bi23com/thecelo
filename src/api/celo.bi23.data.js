...
if(process.argv.length<4)
{
  ...
  if(type == 'get_networkparameters'){
    ...
    if(seconds>0){
      var get_networkparameters_interval = setInterval(get_networkparameters, seconds);
    }
  }
  if(type == 'get_elected_validators'){
    ...
    if(seconds>0){
      var get_elected_validators_interval = setInterval(get_elected_validators, seconds);
    }
  }
  ...
  if(type == 'group_info'){
    ...
    if(seconds>0){
      var group_info_interval = setInterval(group_info, seconds,group_address,redis_client);
    }
  }
}
