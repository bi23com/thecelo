...
function update_validators_balance() {
  ...
  redis.redis_client.set(
    "validators_balance",
    JSON.stringify(validators_balance)
  );
...
function account_balance(address) {
  ...
  balances["lockedGold"] = parseFloat(
    lines[2].trim().replace("lockedGold: ", "")
  );
  ...
  lines.forEach(function (line) {
    if (line.indexOf("0x") == 0) {
      ...
      for (var i = 1; i < len - 2; i++) {
        if (name.indexOf("cLabs") == -1) name += values[i] + " ";
      }
      name = name.trim();
  ...
  try {
    ...
    lines.forEach(function (line) {
      if (line.indexOf("0x") == 0) {
        ...
        //
        if (groups[affiliation]) groups[affiliation][3]++;
      }
    });
  } catch (err) {
    if (err) throw err;
...
async function update_validator_status_all() {
  ...
  //
  var election_current = await redis.get_redis_data(
    theceloconst.election_current_key
  );
  if (election_current) {
  ...
  try {
    ...
    lines.forEach(function (line) {
      if (line.indexOf("0x") == 0) {
        ...
        var score = 0;
        if (validators_score[address]) score = validators_score[address][1];
        var logo = logoExists(address);
        //
        var metadata_url = "";
  ...
  redis.redis_client.set(
    theceloconst.validators_key,
    JSON.stringify({ timestamp, validators })
  );
  redis.redis_client.publish(
    theceloconst.validators_key,
    theceloconst.validators_key,
    function (err, result) {}
  );
...
async function celocli_group_info(groups, group_address) {
  ...
  if (data) {
    ...
    try {
      ...
      var timestamp = new Date().getTime();
      redis.redis_client.set(
        "group_" + group_address,
        JSON.stringify({ timestamp, group })
      );
    } catch (err) {
      if (err) throw err;
    }
...
async function update_groups_list() {
  ...
  //
  redis.redis_client.set(
    theceloconst.groups_key,
    JSON.stringify({ timestamp, groups })
  );
  redis.redis_client.set(
    theceloconst.election_current_key,
    JSON.stringify({ timestamp, election_current })
  );
  redis.redis_client.publish(
    theceloconst.groups_key,
    theceloconst.groups_key,
