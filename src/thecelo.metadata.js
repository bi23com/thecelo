...
const TGSCO_addresses = {
  ...
  "0xF83C93ea360B66DDCD532960304948B1c10786a1": [
    "dsrv labs - WellDoneStake.com",
    "75.00%",
    "https://www.celowhale.com",
  ],
  ...
  "0x8493BD3De67AC341D4cC11531f96a1A2cdBf29aD": [
    "ryabina-1",
    "0.00%",
    "https://celomap.io | https://t.me/Celo_Ryabina_bot",
  ],
};
...
//
const releaseGold_url_1 =
  "https://gist.githubusercontent.com/timmoreton/e60676b96bfd507228e9e56d718bc750/raw/f83fb3738ee6f759118d6e8ba98490f2e9696560/ValidatorReleaseGold.json";
const releaseGold_url_2 =
  "https://gist.githubusercontent.com/timmoreton/27e975bbca63723e218288b1a1f9fa54/raw/d55f51fde470fe040407c02462aeb5a7adc3d57f/CeloRC1ReleaseGoldWave2.json";
//
async function getMetaInfo(
  ...
  if (ReleaseGold_address.length == 0) {
    ...
    if (data) {
      ...
      if (data) {
        ...
        ReleaseGold_address = releaseGold_address_1.concat(
          releaseGold_address_2
        );
        redis.redis_client.set(
          "releaseGold_address",
          JSON.stringify(ReleaseGold_address)
        );
      }
    }
  }
  ...
  var metadataURL;
  //
  var groups_metadata = await redis.get_redis_data(
    theceloconst.groups_metadata_key
  );
  ...
  */
  ...
  if (group) {
          ...
          try {
            ...
            if (metadata && metadata.claims) {
              metadata.claims.forEach((item, ii) => {
                ...
                if (item.type == "ATTESTATION_SERVICE_URL")
                  result.attestation_service_url = item.url;
              });
            }
          } catch (err) {
            console.log(url);
    ...
    if (elected_count * 1e24 == epoch_scores) {
  ...
  if (votes_result) {
    ...
    var foundationVotes_address = JSON.stringify(
      FoundationVotes_address
    ).toLowerCase();
    votes.forEach((vote, i) => {
      if (vote[1].toLowerCase() == address.toLowerCase()) {
        if (
          foundationVotes_address.indexOf(vote[0].toLowerCase()) >= 0 &&
          vote[2] > 0
        ) {
          //Receives Celo Foundation votes
          if (identify[9].trim().length > 0) identify[9] += " , ";
          identify[9] += vote[0] + ": " + parseFloat(vote[2] / 1e18).toFixed(4);
        }
      }
    });
  }
  ...
  if (key) {
    //Can receive votes
    identify[1] = parseFloat(groups[key][2] / 1e18).toFixed(2);
  }
  //Secure
  if (thecelo.containsKey(TGSCO_addresses, address)) {
    ...
    if (
      parseFloat(
        TGSCO_addresses[address][1].replace("/", "0").replace("%", "")
      ) *
        1.3 >=
      80
    )
      identify[4] =
        "TGCSO master validator chanllege: " + TGSCO_addresses[address][1];

    if (
      TGSCO_addresses[address][2] &&
      TGSCO_addresses[address][2].trim().length > 0
    )
  ...
  if (groups[key][0].trim().length > 0) {
    var url =
      "http://baklava.thecelo.com/api/?method=find_group&name=" +
      encodeURI(groups[key][0]);
    console.log(url);
