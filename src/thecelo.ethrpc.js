...
function getAddresses(count, earliest = false) {
  redis.redis_client.get(theceloconst.accounts_key, function (err, data) {
    if (!err && data) {
      ...
      if (!earliest) accounts_height = obj.blockHeight;
    ...
    */
    ...
    for (
      ;
      accounts_height <= fromHeight + count &&
      accounts_height <= lastestBlockNumber;
      accounts_height++
    ) {
      //
      if (parseInt(accounts_height) % 17280 == 5) updateAllBalance = true;
      //
      var block = thecelo.eth_rpc(
        "eth_getBlockByNumber",
        '["0x' + accounts_height.toString(16) + '",true]'
      );
      ...
      if (block) {
        ...
        for (let i = 0; i < txs.length; i++) {
          ...
          if (tx["from"] && tx["to"]) {
            var from = tx["from"];
            if (!addressesList[from]) addressesList[from] = [0, 0, 0];
            addressesList[from][0]++;
            //
            var to = tx["to"];
            if (!addressesList[to]) addressesList[to] = [0, 0, 0];
            addressesList[to][0]++;
    ...
    Object.keys(addressesList).forEach(function (address) {
      if (addressesList[address][0] > 0)
        counts["txs"] += addressesList[address][0];
    ...
    var result = thecelo.eth_rpc(
      "eth_getLogs",
      '[{"fromBlock": "0x' +
        fromHeight.toString(16) +
        '","toBlock":"0x' +
        accounts_height.toString(16) +
        '","address":"' +
        cgld_address +
        '","topics":["' +
        transfer_topic +
    ...
    //totalSupply
    ////////////////////////////////////////////////
    var cGLD_totalSupply = thecelo.eth_rpc(
      "eth_call",
      '[{"to": "' +
        cgld_address +
        '", "data":"' +
    ...
    if (cGLD_totalSupply == null || cGLD_totalSupply == "0x")
      cGLD_totalSupply = 0;
    cGLD_totalSupply = cGLD_totalSupply / 1e18;
    ...
    var cUSD_totalSupply = thecelo.eth_rpc(
      "eth_call",
      '[{"to": "' +
        cusd_address +
        '", "data":"' +
    ...
    if (cUSD_totalSupply == null || cUSD_totalSupply == "0x")
      cUSD_totalSupply = 0;
    cUSD_totalSupply = cUSD_totalSupply / 1e18;
    eth_dataset["cUSD_totalSupply"] = cUSD_totalSupply;
    //
    redis.redis_client.set(
      theceloconst.eth_dataset_key,
      JSON.stringify(eth_dataset)
    );
    ////////////////////////////////////////////////
    ////////////////////////////////////////////////
    //
    var jsonString =
      ...
      accounts_height +
      ',"cGLD_transfers":' +
      eth_dataset["cGLD_transfers"] +
      ',"cUSD_transfers":' +
      eth_dataset["cUSD_transfers"] +
      ',"addresses":' +
...
function updateBalance(address) {
  //
  var cgld = ethweb3.getAccountInfo(
    cgld_address,
    "balanceOf",
    address,
    "uint256"
  );
  if (parseInt(cgld) > 1e50) {
    cgld = 0;
  }
  var cusd = ethweb3.getAccountInfo(
    cusd_address,
    "balanceOf",
    address,
    "uint256"
  );
  addressesList[address][1] = cgld / 1e18;
  addressesList[address][2] = cusd / 1e18;
  ...
  addressesList[address][4] = account.lockedGold / 1e18;
  addressesList[address][5] = account.nonVotingLockedGold / 1e18;
  addressesList[address][6] = account.pendingWithdrawals / 1e18;
