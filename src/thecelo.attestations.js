...
function getAttestationLogs() {
  ...
  result.forEach((item, i) => {
    ...
    attestationsRequestedLogs.unshift({
      blockNumber,
      identifier,
      account,
      attestationsRequested,
  ...
  result = thecelo.eth_rpc(
    ...
    '[{"fromBlock": "' +
      ...
      attestations_proxy +
      '","topics":["' +
      attestationIssuerSelected_topic +
      '"]}]'
  );
  result.forEach((item, i) => {
    let identifier = web3.eth.abi.decodeParameter("bytes32", item.topics[1]);
    ...
    let attestationRequestFeeToken = web3.eth.abi.decodeParameter(
      "address",
      item.data
    );
    let blockNumber = item.blockNumber;
    attestationIssuerSelectedLogs.unshift({
      blockNumber,
      identifier,
      account,
      issuer,
  ...
  result.forEach((item, i) => {
    ...
    let issuer = web3.eth.abi.decodeParameter("address", item.topics[3]);
    let blockNumber = item.blockNumber;
    attestationCompletedLogs.unshift({
      blockNumber,
      identifier,
      account,
      issuer,
  ...
  let attestation_logs = {
    attestationsRequestedLogs,
    attestationIssuerSelectedLogs,
    attestationCompletedLogs,
  };
...
function getAttestationInfo(address) {
  if (Object.keys(attestationInfos).length == 0) {
    getAttestationLogs();
  }
  //Attestations fulfilled/requested
...
async function getEpochAttestation() {
  ...
  requestedLogs.forEach((item, i) => {
    ...
    //
    if (epochCount[epoch])
      epochCount[epoch][0] += parseInt(item.attestationsRequested);
    else
      epochCount[epoch] = [parseInt(item.attestationsRequested), 0, 0, 0, 0, 0];
    //
